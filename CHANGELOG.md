...
- The Toggle Developer Tools command (⌘⌥I) now docks eruda as a bottom pane instead of floating as an overlay. A new `DevToolsPane` in `src/views/main/App.tsx` sits between the main workspace area and the `UsageFooter`, so the layout is `[ViewModeBar] / [main content] / [eruda pane 50vh] / [UsageFooter]` when devtools are open. Eruda is initialized with `inline: true` (`src/views/main/state/devtools.ts`), which switches its dev-tools panel from `position: fixed` to `position: static` via eruda's own `._inline{position:static}` rule. The panel's descendants still use `position: absolute`, so a positioned ancestor is required as a containing block. The `DevToolsPane` uses **two** nested elements for this: an outer wrapper (holds `height: 50vh` + `overflow: hidden` + **`position: relative`**) and an inner host that is passed to `eruda.init()`. The wrapper gets the `position: relative` because eruda's `:host { all: initial }` shadow-root rule wipes *any* `position` value set on the host itself — verified via diagnostic logging, even inline `!important` JS `setProperty` was observed reset to `all: initial`. Eruda does not touch the outer wrapper, so its `position: relative` survives and serves as the containing block for the absolute-positioned dev-tools descendants. A `devtoolsVisible` flag on the Zustand store drives visibility; `toggleDevTools` just flips the flag. The pane stays mounted after first use (unmounting would orphan eruda's shadow root) — hiding collapses the wrapper height to `0`. Eruda is lazy-imported on first open so startup is unaffected.

### Added

- Added two command palette entries — "Open User Claude Settings" and "Open Workspace Claude Settings" — that open `~/.claude/settings.json` and `<workspace>/.claude/settings.json` respectively in an Editor tab. User-path resolution reuses the existing `browsePath` RPC to expand `~`; the workspace command no-ops if no workspace is selected. If the workspace `.claude/settings.json` doesn't exist yet, a new `CreateClaudeSettingsDialog` (`src/views/main/components/palette/CreateClaudeSettingsDialog.tsx`) asks whether to create it — confirming writes `{}\n` via `writeFileForEditor` (Bun.write auto-creates the `.claude` parent directory) and then opens it. Dialog visibility is driven by a `createClaudeSettingsRequest` slot on the Zustand store (callback-in-state, consistent with the existing one-off dialog pattern like `cloneRepoDialogVisible`). Both commands support open-as-pane (⇄) via ←/→ in the palette.
- Added a file tree sidebar view that complements the Cmd+P command palette for file opening. A new activity bar column (40px, fixed width, always visible) lives to the left of the resizable sidebar with two icons — Workspaces (top) and Files (bottom) — that toggle which view fills the sidebar. Clicking the active icon hides the sidebar; clicking an inactive icon shows it and switches the view. The Files view renders a unified 3-level tree — repository → workspace → files/folders — mirroring IntelliJ's multi-project workspace pattern; top levels come from the existing `repos` / `workspacesByRepo` store, file levels lazy-load via a new `listDir` RPC (`src/bun/file-tree/file-tree-service.ts`) that returns structured `DirEntry[]` objects. Clicking a file opens it via `openFileInWorkspace` in `src/views/main/state/actions.ts`: the helper first scans every workspace's pane tree for an existing tab whose `editorFilePath`/`markdownFilePath` matches the clicked absolute path — if found, it switches the focused workspace to that tab's owner and refocuses the existing tab (no duplicate). Otherwise it falls through to the cross-workspace switch+open pattern copied from `openPlanInWorkspace` in `ProgressRowDetail.tsx`, which handles the case where the clicked file lives in a workspace whose pane tree hasn't been materialized yet. Markdown files open in a MarkdownViewer tab; everything else opens in an Editor tab. Visual indicators: a green dot marks the currently-focused workspace row, and the row matching the focused pane's selected tab gets a blue left-edge accent + subtle background tint. Keyboard navigation lives on the tree container (single `tabIndex=0`, ARIA `role="tree"`): arrows move/collapse/expand, Enter opens, Home/End jump. Expansion state, cursor, `activeSidebarView`, and scroll position persist across reloads via a new `fileTree` field on `SessionState` (additive, no version bump) + a `saveFileTreeState` RPC that debounces writes through the webview. Live filesystem updates use one recursive `fs.watch(workspacePath, {recursive: true})` per expanded workspace with a 100ms per-directory debounce (`src/bun/file-tree/file-tree-service.ts`); the `directoryChanged` push event invalidates the matching `fileTreeEntries` cache entry and refetches if the dir is still expanded. Watchers are only alive while the Files view is the active sidebar content — toggling away calls `unwatchAllDirectoryTrees`, and toggling back re-registers watchers for every expanded workspace and invalidates every expanded-dir cache so missed events don't leave the tree stale. **Bug fix during live testing:** the `useEffect` in `FileTreeView.tsx` that dispatches `listDir` had `entries` / `loading` / `errorMap` in its dependency array, so calling `setFileTreeLoading(path, true)` changed the `loading` state, re-ran the effect, and its cleanup set `cancelled = true` — cancelling the in-flight RPC before it could populate entries and leaving newly-expanded folders stuck on "Loading…" forever. Deps are now scoped to `[expandedWorkspaces, expandedDirs]` and the cache is read via `useStore.getState()` inside the effect, so cancellation only fires on genuine user actions (collapsing mid-fetch), not on self-induced loading-state flips. New files: `src/views/main/components/sidebar/ActivityBar.tsx`, `FileTreeView.tsx`, `FileTreeNode.tsx`, `src/bun/file-tree/file-tree-service.ts`.
- Phase 2 polish on the file tree sidebar. **Gitignore awareness:** entries matching `.gitignore` / `.jj-ignore` rules (or one of the always-ignored directory names like `node_modules`, `dist`, `.git`) now render at 50% opacity rather than being filtered out — they remain browsable but de-emphasized. Matching uses the `ignore` npm package; each workspace's matcher combines every `.gitignore` encountered between the workspace root and the directory being listed (handles nested `.gitignore` files), and the cache is invalidated when the watcher sees a `.gitignore` mutation. **VCS status decorations:** a colored letter badge (M / A / D / R / U, color-coded) appears on each file row that shows up in `getVCSStatus`; workspace rows get a yellow roll-up dot when any file below them is changed. The status is fetched lazily per expanded workspace and refreshed whenever `fileTreeEntries` invalidates (i.e., on the same signal that drives the watcher-triggered re-fetch). **Inline filter box:** a small search input at the top of the tree fuzzy-matches node labels using the existing `fuzzyMatch` helper; matches keep their ancestor rows visible so the hit stays in context. `Esc` clears the filter. **Reveal-active button:** expands the currently-focused tab's file's ancestors (repo → workspace → intermediate dirs) and moves the cursor to it; disabled when no file is focused. **Right-click context menu:** Open in Split, Reveal in Finder (via a new `revealInFinder` RPC that runs `open -R`), Copy Path, Copy Relative Path. Workspace and directory rows omit the split/relative entries that don't apply. A new `openFileInSplit` helper in `actions.ts` creates a new pane adjacent to the focused one with the file's tab inside — skipping the tab-dedup check since "open in split" explicitly wants a second view. **Hidden-files toggle:** a button in the tree header switches between "dim dotfiles + ignored files" (default) and "render everything at full opacity". Persisted in session state. **Drag-to-pane / drag-to-terminal:** file rows are `draggable`; the drag payload is a new `FILE_TREE_DRAG_MIME` JSON object plus a `text/plain` fallback. `PaneDropZone` now activates for either tab-drags or file-drags and shows a center-drop indicator for files; dropping in the middle of a pane opens the file there, dropping near the left/right edge creates a split with the file in a new pane. Terminal panes intercept file drops via a simple overlay and write a shell-single-quoted path to the PTY, so you can drag a file onto a Claude / shell tab to insert its path at the prompt.
...
- Fixed PR review failing on jj repos in `src/bun/vcs/jj-provider.ts`. `PRReviewCoordinator` fetches the PR head via `git fetch origin +refs/pull/<n>/head:refs/heads/tempest/pr-<n>`, which creates a *local* branch (so the fork PR case works too) — it does not create a remote-tracking ref. `JJProvider.createWorkspace` then ran `jj git fetch -b <branch>` followed by `jj new <branch>@origin`, but `<branch>@origin` doesn't exist (origin has no `tempest/pr-<n>` branch, only `refs/pull/<n>/head`), producing `Error: Revision 'tempest/pr-<n>@origin' doesn't exist`. Changed the import step to `jj git import` (just sync jj's view of git refs — no remote fetch, which was trying to re-fetch a branch that isn't on origin) and switched the new-commit target from `<branch>@origin` to the local bookmark `<branch>`, matching jj's own error hint and the ref the coordinator actually produces.
- Fixed jj workspaces with spaces (or other special characters) in their names not appearing in the sidebar. `JJProvider.listWorkspaces` in `src/bun/vcs/jj-provider.ts` parsed `jj workspace list` output by slicing up to the first `:` and using the result verbatim as both the display name and the directory name. jj wraps names containing whitespace/special characters in double quotes (e.g. `"PR review in JJ": <change-id> ...`), so the parser produced `"PR review in JJ"` (with literal quotes) as the name, and `existsSync(join(wsRoot, name))` in `WorkspaceManager.refreshWorkspacesInternal` then failed against the real directory (which has no quotes), dropping the workspace from the sidebar. Introduced a `parseWorkspaceName` helper that detects the leading `"`, walks to the matching close quote (honoring backslash escapes), and falls back to the previous colon-split behavior for unquoted names.
### Changed
