...
- Fixed terminal-driven tab updates being silently dropped for terminals in background (non-selected) workspaces in `src/views/main/state/actions.ts`. `updateTabLabelByTerminalId`, `updateTabCwdByTerminalId`, and `updateTabProgressByTerminalId` previously used the `currentTree()` helper, which only returns the pane tree for `selectedWorkspacePath`. Because background workspaces' terminals are kept mounted via the opacity/pointer-events pattern, title/CWD/progress events routed by `terminalId` from those background terminals would miss the lookup and be discarded. Added a shared `findWorkspaceWithTerminal(terminalId)` helper that iterates `useStore.getState().paneTrees` entries to locate the workspace containing the matching tab, and updated all three terminal-event-driven functions to use it so each event updates its owning workspace's tree (not necessarily the selected one).
...
- Fixed VCS view losing its internal state (scroll position, selected file, staged/partial diffs) on every view-mode switch in `src/views/main/components/layout/WorkspaceDetail.tsx`. `VCSView` was conditionally mounted via `{viewMode === ViewMode.VCS && <VCSView />}` inside the absolute-positioned wrapper, which unmounted and remounted it whenever the user toggled to Terminal or Dashboard. It is now rendered unconditionally inside the same `absolute inset-0` + `opacity-0 pointer-events-none` wrapper used by Terminal and Dashboard, matching the CLAUDE.md lifecycle rule that tab-switched views should be hidden rather than unmounted.
