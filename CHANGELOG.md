...
- `tsconfig.json` for the Tempest repo: added `exclude: ["build", "dist", ".log", "node_modules"]`. Without this, tsserver's "non-TS file size budget" (default 20 MB) overflowed because of the bundled output in `build/` (a 14 MB `bun/index.js` plus several multi-MB Monaco workers). On overflow, tsserver disables the language service for the whole project — which works for hover and document symbols (which use lighter paths) but breaks rename, find-references, and any operation needing cross-file project info. This is a one-time tsconfig fix; new projects will need similar excludes if their build outputs land near tsserver's limit.
