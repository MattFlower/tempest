...
export class CodexHistoryMetadataCache {
  ...
  private walkForRollouts(root: string, out: string[]): void {
    ...
    for (const entry of entries) {
      // Skip symlinks to avoid unbounded recursion if the sessions tree
      // contains a loop (e.g. a user-created symlink back to the root).
  ...
  private extractMetadata(filePath: string): {
    ...
    // Walk every line in the ~256 KiB we read. The byte cap already
    // bounds the cost; what we want here is to not miss the first user
    // prompt when the preamble (reasoning, tool calls, long session_meta)
    // is bigger than an arbitrary line count.
